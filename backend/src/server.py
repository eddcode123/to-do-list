...
class NewListReponse(BaseModel):
    id: str
    name: str
...
async def create_todo_list(new_list: NewList) -> NewListReponse:
    return NewListReponse(
...
class DummyReponse(BaseModel):
    id: str
    when: datetime


@app.get("/api/dummy")
async def get_dummy() -> DummyReponse:
    return DummyReponse(
        id=str(ObjectId()),
        when=datetime.now()
    )
